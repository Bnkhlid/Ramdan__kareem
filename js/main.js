...
window.spaCore = {
  ...
  async loadPage(url, push = true) {
    ...
    try {
      ...
      // RC#7: Strip body inline style before swap to prevent CSS bleed (e.g. tracker dark override)
      document.body.removeAttribute('style');

      // RC#1: Remove previous page's injected styles, then inject new page's styles
  ...
  async executeScripts(newDoc) {
    ...
    document.addEventListener = function (type, listener, options) {
      if (type === 'DOMContentLoaded') {
        // RC#3: Use 50ms delay (was 1ms) so all function declarations in the script block are parsed first
        setTimeout(() => {
